...
def normalized_adj_tensor(a:Tensor):
    a = a + torch.eye(a.shape[0])
    deg_in = torch.pow(a.sum(0), -0.5)
    deg_in[torch.isinf(deg_in)] = 0.
    deg_mat_in = torch.diag(deg_in)
    return a.matmul(deg_mat_in).matmul(deg_mat_in.T)
...
def load_citation(dataset_str="cora"):
    ...
    for i in range(nodes):
        for j in range(len(graph[i])):
            node = graph[i][j]
            a[i][node] = 1
    test_idx_reorder = parse_index_file("dataset/"+dataset+"/raw/ind.{}.test.index".format(dataset_str))
...
def load_ppi():
    ...
    train_feats = features_[train_ids[:, 0]]  ##根据下标获取训练测试验证集
    ...
    splits = dfs_split(scipy.sparse.csr_matrix(adj))
    ...
    dict_splits = find_split(scipy.sparse.csr_matrix(adj), splits, g_data['nodes'])
...
def load_rcaeval_from_dir(dataset_name):
    import numpy as np
    import torch
    import scipy.sparse as sp
    ...
    data_dir = Path(__file__).resolve().parent / "dataset" / dataset_name

    features = np.load(data_dir / "features.npy").astype(np.float32)
    labels = np.load(data_dir / "labels.npy").astype(np.int64)
    idx_train = np.load(data_dir / "idx_train.npy").astype(np.int64)
    idx_val = np.load(data_dir / "idx_val.npy").astype(np.int64)
    idx_test = np.load(data_dir / "idx_test.npy").astype(np.int64)

    adj = load_npz(data_dir / "adj.npz").astype(np.float32)

    # 与原 citation 数据保持一致：加 self-loop 并做归一化
    adj = adj + sp.eye(adj.shape[0], dtype=np.float32)
    adj_normalized = normalize(adj)
    ...
    adj_raw_torch = sparse_mx_to_torch_sparse_tensor(adj)

    return adj_torch, features, labels, idx_train, idx_val, idx_test, adj_raw_torch


def load_rcaeval_ob():
    return load_rcaeval_from_dir("RCAEval_OB")


def load_rcaeval_ss():
    return load_rcaeval_from_dir("RCAEval_SS")


def load_rcaeval_tt():
    return load_rcaeval_from_dir("RCAEval_TT")
