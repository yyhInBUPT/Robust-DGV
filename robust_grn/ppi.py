...
parser.add_argument('--dims', type=list, default=[2048], help='conv dims')
...
model = RobustGRNModel_PPI(nfeat=train_feat[0].shape[1],
                    nlayers=args.layer,
                    dim=args.dims,
...
for epoch in range(args.epochs):
    ...
    if acc_val > acc:
        acc = acc_val
        best_epoch = epoch
        torch.save(model.state_dict(), checkpt_file+'ppi')
        bad_counter = 0
    else:
        bad_counter += 1
